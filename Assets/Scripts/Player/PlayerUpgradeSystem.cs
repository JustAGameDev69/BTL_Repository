...
public class PlayerUpgradeSystem : MonoBehaviour
{
    ...
    public int playerHealthUpgrade = 100;
    ...
    public void OnClickUpgradeHealth()
    {
        if (CheckEnoughCoins(upgradeHealthPrice))
        {
            ...
            playerStats.SetHealth(playerCombat.playerHealth += 20);
            Debug.Log(playerCombat.playerHealth);
    ...
    public void OnClickUpgradeAttackDamage()
    {
        if (CheckEnoughCoins(upgradeATDPrice))
        {
            ...
            PlayerCoinUpdate();
            playerStats.SetDamage(playerCombat.attackDamage += 2);
            upgradeATDPrice += 2;
            PlayerDamage();
        }
        else
    ...
    public void OnClickUpgradeAttackRange()
    {
        if (CheckEnoughCoins(upgradeATRPrice))
        {
            ...
            PlayerCoinUpdate();
            playerStats.SetAttackRange(playerCombat.attackRange += 0.05f);
            upgradeATRPrice += 2;
            PlayerAttackRange();
        }
        else
    ...
    public void OnClickUpgradeAttackSpeed()
    {
        if (CheckEnoughCoins(upgradeATSPrice))
        {
            ...
            PlayerCoinUpdate();
            playerStats.SetAttackSpeed(playerCombat.attackSpeed += 0.1f);
            upgradeATSPrice += 2;
            PlayerAttackSpeed();
        }
        else
    ...
    public void OnClickUpgradeMoveSpeed()
    {
        if (CheckEnoughCoins(upgradeMovePrice))
        {
            ...
            PlayerCoinUpdate();
            playerStats.SetMovementSpeed(playerController.speed += 0.2f);
            upgradeMovePrice += 2;
            PlayerMoveSpeed();
        }
        else
